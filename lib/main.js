...
var searchPageMod = pageMod.PageMod({
  ...
  onAttach: function(worker) {
    worker.port.on('search', function(word) {
      ...
      console.log(simpleStorage.storage.localSearchUrls);
      var result = {};
      var index;
      for (index in storedResult){
        result[simpleStorage.storage.localSearchUrls[index]] =
          storedResult[index];
      }
      console.log(result);
...
function storeData(data){
  ...
  console.log(simpleStorage.storage.localSearch["article"]);
}
